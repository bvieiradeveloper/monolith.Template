...
namespace MonolithTests.Invoice
{
    public class GenerateInvoiceUseCaseTest
    {
        ...
        public GenerateInvoiceUseCaseTest()
        {
        ...
        [Fact]
        public async Task ShouldGenerateInvoice()
        {
            var invoiceRepository = new Mock<IInvoiceRepository>();

            invoiceRepository
                .Setup(s => s.Generate(It.IsAny<InvoiceEntity>()))
                .Returns(() => Task.FromResult(_invoice));

            var useCase = new GenerateInvoiceUseCase(invoiceRepository.Object);

            var response = await useCase.Execute(new GenerateInvoiceInputDto
            {
